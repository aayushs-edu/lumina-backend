...
def generate_policy():
    ...
    if os.path.exists(stored_naps_path):
        ...
        if not existing_row.empty:
            ...
            structured_content = extract_chunks(output)
            return jsonify({"output": structured_content})

    print(f"\n▶ Generating NAP for: {country}")

    output = generate_nap_for_country(df, country, focus_areas, stories, alternate=False)
    ...
    # Extract structured content from the output
    structured_content = extract_chunks(output)
...
def enhance_policy():
    ...
    output = generate_nap_for_country(df, country, focus_areas, stories, alternate=True)
...
def generate_nap_for_country(df, country, focus_areas, stories, alternate):
    print(f"\n▶ Generating NAP for: {country}")
    ...
    prompt = build_formatted_prompt(country, row, df, similar, comparison, alternate, focus_areas, stories)
...
def extract_chunks(output):
    """
    Extract structured content from the raw output based on:
    - Main sections (surrounded by double asterisks **)
    - Subsections (surrounded by single asterisks *)
    Each chunk includes its header and corresponding content with properly structured subsections.
    """
    chunks = []
    lines = output.split("\n")
    current_section = {"header": None, "content": "", "subsections": []}
    current_subsection = None
    in_subsection = False
    
    for line in lines:
        # Check for main section headers (surrounded by double asterisks)
        if line.strip().startswith("**") and line.strip().endswith("**"):
            # Save the previous section if it exists
            if current_section["header"]:
                # Clean up the final section's content
            ...
            # Start a new section
            current_section = {"header": line.strip().strip("**").strip(), "content": "", "subsections": []}
            current_subsection = None
            in_subsection = False
            
        # Check for subsection headers (surrounded by single asterisks)
        elif line.strip().startswith("*") and line.strip().endswith("*") and not (line.strip().startswith("**") or line.strip().endswith("**")):
            # Save previous subsection if it exists
            if current_subsection:
                current_section["subsections"].append(current_subsection)
                
            # Start a new subsection
            subsection_title = line.strip().strip("*").strip()
...
def build_formatted_prompt(country, row, df, similar_df, comparison_metrics, alternate, focus_areas, stories : list):
    ...
    if alternate:
        ...
        **You are a policy assistant tasked with refining and strengthening a country’s existing National Action Plan (NAP) for gender equality. 
        PLEASE STRUCTURE YOUR RESPONSE IN SECTIONS AND SUBSECTIONS, WHERE SECTION HEADERS ARE SURROUNDED BY DOUBLE ASTERISKS AND SUBSECTION HEADERS ARE SURROUNDED BY SINGLE ASTERISKS.
